...
import { WordSearchResult } from "./Components/WordSearchResult";
...
function App() {
  ...
  const handleSearch = () => {
    const checkIfWord = /\w/g;
    if (userSearch === null || !userSearch.match(checkIfWord)) return;
    const whiteSpace = /^\s+|\s+$/g;
  ...
  return (
    <div className="App">
      ...
      {wordFound === undefined ? (
        ...
        <WordSearchResult
          add={handleNotFoundAdd}
          word={userSearch}
          position={wordPosition}
          searchResult={wordFound}
        />
      )}
